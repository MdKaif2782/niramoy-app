...
export const triggerNotification = async (title: string, body: string,route?:string) => {
  ...
  if (remoteMessage.from === "/topics/blood-request") {
    ...
    if (remoteMessage.data) {
      ...
      if (distance <= limit) {
        ...
        await triggerNotification(title, body,`/donor-blood-request?bloodRequestId=${bloodRequest.id}`);
      }
    }
  }
